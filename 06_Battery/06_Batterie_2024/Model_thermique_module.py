...
longueur_tot = (cell_d + marge_cell) * math.cos(30 * math.pi / 180) * (serie - 1) + (
    cell_d + marge_cell
)
...
T_ini = 20.0  # °C (température initiale air)
T_max = 58.0  # °C (température maximale admissible)
...
def heat_spec_capacity(
    T,
):  # https://www.sciencedirect.com/science/article/pii/S1359431123012413
    return 2.08 * T + 812.16


def equation(
    h, deltaS, T_cell, T_air
):  # https://www.sciencedirect.com/science/article/pii/S0378775305013054#eq3
    return (
        Q + deltaS * cell_I / F * (T_cell + 273.15) - h * cell_S * (T_cell - T_air)
    ) / (cell_m * heat_spec_capacity(T_cell))


def air_thermal_conductivity(
    T,
...
def area_busbar(T, t):
    return (
        parallele
        * cell_I
        * math.sqrt(resi_cuivre * t / (cp_cuivre * rho_cuivre * (T - T_ini)))
    )
...
def simple_render(
    t=0,
):  # Fonction de génération des images pour le gif (version précédente du programme)
    circles_cell = []
    circles_air = []
...
def animate(
    frame, axes, frames, tini, tfin
):  # Même fonction que précédemment, mais améliorée
...
def create_animation(
    tini=0, tfin=t_final, frames=150, fps=15
):  # Fonction de génération de toute l'animation à partir de "animate"
    fig, ax = plt.subplots(
        figsize=(int(5 * longueur_tot / largeur_tot), 5)
    )  # note we must use plt.subplots, not plt.subplot
    cmap = mpl.colors.ListedColormap(
        ["cyan", "lawngreen", "yellow", "orange", "red"]
    ).with_extremes(under="blue", over="darkred")
    ...
    ani = animation.FuncAnimation(
        fig,
        partial(animate, axes=ax, frames=frames, tini=tini, tfin=tfin),
        frames=frames,
    )
...
def heat_transfer_coeff(
    v, T, L
):  # Dittus–Boelter equation https://www.sciencedirect.com/topics/engineering/dittus-boelter-correlation
    Nu = (
        0.023 * np.abs(Re(v, L)) ** (0.8) * np.abs(Pr_air(T)) ** 0.4
    )  # Nusselt number (voir aussi https://en.wikipedia.org/wiki/Nusselt_number)
    ...
    for k in range(N - 1):
        for x in range(serie):
            ...
            if x == 0:
                ...
                q = (
                    heat_transfer_coeff(v_air, T_air_left, L)
                    * cell_S
                    * (T_cell_left - T_air_left)
                )
            v_air = Qv / (largeur_tot * cell_h - cell_d * parallele * cell_h) / 3600
            L = largeur_tot - cell_d * parallele
            T_air = (
                T_air_left + q / Qv / air_density(T_air_left) / r_air / cp_air * 3600
            )
